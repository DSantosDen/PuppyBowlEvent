...
// 01 Log the initial state
console.log("Initial state: ", store.getState());

// 02 Every time the state changes, log it
let unsubscribe = store.subscribe(() =>
  console.log("State after dispatch: ", store.getState())
);

// 03 Dispatch an action
store.dispatch(puppyBowlApi.endpoints.fetchPlayers.initiate());

// 04 Stop listening to state updates
unsubscribe();
